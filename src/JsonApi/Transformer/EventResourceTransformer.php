...
class EventResourceTransformer extends AbstractResource
{
    ...
    {
        return (string) $event->getEventid();
    }
    ...
    public function getAttributes($event): array
    {
        return [
            ...
            'datecreated' => function (Event $event) {
                return ($event->getDatecreated()) ? $event->getDatecreated()->format(DATE_ATOM) : NULL;
            },
            'datemodified' => function (Event $event) {
                return ($event->getDatemodified()) ? $event->getDatemodified()->format(DATE_ATOM) : NULL;
            },
            'dateevent' => function (Event $event) {
                return ($event->getDateevent()) ? $event->getDateevent()->format(DATE_ATOM) : NULL;
            },
